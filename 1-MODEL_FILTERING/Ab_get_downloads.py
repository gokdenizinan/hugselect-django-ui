...
def get_model_list(model_dict):
    return [item["model_id"] for item in model_dict.values()]

model_ids = get_model_list(model_dict)  # Limit to first 5000 for testing
...
with open("1-MODEL_FILTERING/hf_model_download_stats.json", "w", encoding="utf-8") as f:
    json.dump(download_dic, f, indent=2, ensure_ascii=False)

with open("1-MODEL_FILTERING/hf_model_all_stats.json", "w", encoding="utf-8") as f:
    json.dump(model_stats, f, indent=2, ensure_ascii=False)

with open("1-MODEL_FILTERING/hf_gone_models.json", "w", encoding="utf-8") as f:
