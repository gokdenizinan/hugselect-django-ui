...
def build_sample_evaluation_output(
    ...
    *,
    ...
    """
    Create a row-level CSV for manual review with these columns:
      - ModelID
      - Review_Processed
      - ISO Quality   (renamed from prediction_col)
      - chatgpt_primary
      - gemini_primary
      - cumulative
      - c1            (ISO Quality vs cumulative)
      - g1            (ISO Quality vs chatgpt_primary)
      - g2            (ISO Quality vs gemini_primary)
      - errors        (TP / FP / TN / FN)

    Assumptions for the error label:
      - cumulative is the agreed primary label when chatgpt_primary == gemini_primary.
      - if chatgpt_primary and gemini_primary disagree, cumulative is chosen randomly
        from one of the two labels for that row.
      - c1/g1/g2 are "T" when labels match, else "F".
      - any_g = (g1 == "T" or g2 == "T")
      - errors are assigned as:
          TP: c1 == "T" and any_g is True
          FP: c1 == "F" and any_g is True
          FN: c1 == "T" and any_g is False
          TN: c1 == "F" and any_g is False

    Note: because cumulative is always one of the annotator labels, c1 == "T"
    generally implies at least one of g1/g2 is also "T", so FN will usually not occur.
    """
    df = read_table_robust(input_path, preferred_sep=preferred_sep)
    ...
    if missing:
        raise ValueError(f"Missing required columns for sample evaluation export: {missing}")

    out = df[["ModelID", "Review_Processed", prediction_col, "chatgpt_primary", "gemini_primary"]].copy()
    out = out.rename(columns={prediction_col: "ISO Quality"})

    for col in ["ISO Quality", "chatgpt_primary", "gemini_primary"]:
        out[col] = out[col].apply(_safe_str)

    rng = np.random.default_rng()
    agree_mask = out["chatgpt_primary"] == out["gemini_primary"]
    random_pick = rng.integers(0, 2, size=len(out))
    out["cumulative"] = np.where(
        agree_mask,
        out["chatgpt_primary"],
        np.where(random_pick == 0, out["chatgpt_primary"], out["gemini_primary"]),
    )

    out["c1"] = np.where(
        (out["cumulative"] != "") & (out["ISO Quality"] == out["cumulative"]),
        "T",
        "F",
    )
