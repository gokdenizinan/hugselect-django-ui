...
from EE_Query_Builder_fast_bette import ESQueryBuilderAdaptive
from EE_Query_Builder_fast_bette import query_mapping
...
SYNONYM_CACHE_FLUSH_EVERY = 100

_SHARED_ES_CLIENT = None
_SHARED_ES_CLIENT_LOCK = threading.Lock()
_QUERY_VECTOR_CACHE: Dict[str, Optional[List[float]]] = {}
_QUERY_VECTOR_CACHE_LOCK = threading.Lock()
_BUILDER_CACHE: Dict[str, Any] = {}
_BUILDER_CACHE_LOCK = threading.Lock()


def get_shared_es_client():
    global _SHARED_ES_CLIENT
    if _SHARED_ES_CLIENT is not None:
        return _SHARED_ES_CLIENT
    with _SHARED_ES_CLIENT_LOCK:
        if _SHARED_ES_CLIENT is None:
            _SHARED_ES_CLIENT = Elasticsearch(ES_URL)
    return _SHARED_ES_CLIENT


def _flush_builder_caches():
    with _BUILDER_CACHE_LOCK:
        builders = list(_BUILDER_CACHE.values())
    for builder in builders:
        cache = getattr(builder, "synonym_cache", None)
...
atexit.register(_flush_builder_caches)
...
# Grid-only experiment configuration.
# Systematic grid without duplicate configs.
GRID_OPTIONS = {
    "functional_group_scale": [1.2],
    "essential_group_scale": [1.1],
    "preference_group_scale": [1],
    "quality_group_scale": [1],
    "rank_group_scale": [1, 3],
    "rank_max": [20],
    "grams_factor": [0.9],
    "priority_must": [1.8],
...
def encode_query_text(query_text: str, cache_key: Optional[str] = None):
    cache_lookup_key = cache_key or hashlib.sha1(query_text.encode("utf-8")).hexdigest()
    with _QUERY_VECTOR_CACHE_LOCK:
        if cache_lookup_key in _QUERY_VECTOR_CACHE:
            return _QUERY_VECTOR_CACHE[cache_lookup_key]

    encoder = get_vector_encoder(VECTOR_ENCODER_MODEL)
    if encoder is None:
        with _QUERY_VECTOR_CACHE_LOCK:
            _QUERY_VECTOR_CACHE[cache_lookup_key] = None
        return None

    vec = encoder.encode(query_text, normalize_embeddings=True)
    vec_list = vec.tolist() if hasattr(vec, "tolist") else list(vec)
    with _QUERY_VECTOR_CACHE_LOCK:
        _QUERY_VECTOR_CACHE[cache_lookup_key] = vec_list
    return vec_list
...
def instantiate_builder(exp_cfg: ExperimentConfig):
    cache_key = config_signature(exp_cfg.to_dict())
    with _BUILDER_CACHE_LOCK:
        cached_builder = _BUILDER_CACHE.get(cache_key)
    ...
    base_kwargs = dict(
        ...
        synonym_cache_flush_every=SYNONYM_CACHE_FLUSH_EVERY,
    )

    try:
        builder = ESQueryBuilderAdaptive(**base_kwargs)
    except TypeError as e:
        legacy_kwargs = dict(base_kwargs)
        legacy_kwargs.pop("synonym_cache_flush_every", None)
        try:
            builder = ESQueryBuilderAdaptive(**legacy_kwargs)
        except TypeError:
            raise TypeError(
...
def run_bm25_and_optional_vector_search(
    ...
    *,
    es_client=None,
    builder=None,
    prebuilt_groups=None,
    query_vector_cache_key: Optional[str] = None,
) -> Tuple[Dict[str, Any], Dict[str, Any], Any, Any, Any]:
    timer = Timer()
    es_client = es_client or get_shared_es_client()
    builder = builder or instantiate_builder(exp_cfg)
    timer.log("builder + precompute")
    prebuilt_groups = prebuilt_groups if prebuilt_groups is not None else builder.precompute_feature_group_cache(features)
    ...
    timer.log("bm25 search")
    

    hybrid_query = {"bm25": final_query}

    if not exp_cfg.hybrid_search_enabled or exp_cfg.vector_rrf_weight <= 0:
        return bm25_response, hybrid_query, final_feature_groups, builder, prebuilt_groups

    query_text = build_query_text(user_text=user_text, features=features)
    timer.log("build_query_text")
    query_vector = encode_query_text(query_text, cache_key=query_vector_cache_key)
    timer.log("encode_query_text") 
    ...
    try:
        ...
        timer.log("fusion")
        return fused_response, hybrid_query, final_feature_groups, builder, prebuilt_groups
    except Exception as e:
...
def run_single_experiment(
    ...
    print(f"[{eval_id}] Running {exp_cfg.experiment_id}")
    timer.log("start")


    es_client = get_shared_es_client()
    builder = instantiate_builder(exp_cfg)
    timer.log("builder created")
    prebuilt_groups = builder.precompute_feature_group_cache(bundle)
    timer.log("precompute_feature_group_cache")
    query_vector_cache_key = f"{eval_id}::{hashlib.sha1(build_query_text(user_text=user_text, features=bundle).encode('utf-8')).hexdigest()}"

    response, final_query, final_feature_groups, builder, prebuilt_groups = run_bm25_and_optional_vector_search(
        exp_cfg=exp_cfg,
        features=bundle,
        user_text=user_text,
        ...
        builder=builder,
        prebuilt_groups=prebuilt_groups,
        query_vector_cache_key=query_vector_cache_key,
    )
    timer.log("search (bm25 + optional vector)")
...
for original_key, original_paper in rationale_input.items():
    ...
    sample_year_1 =original_paper.get("year", "unknown_year")
    sample_year = str(int(sample_year_1) + 1)
