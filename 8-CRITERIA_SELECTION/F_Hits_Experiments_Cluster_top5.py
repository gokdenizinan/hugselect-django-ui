...
# -----------------------
# 11) TOP 5 EXPERIMENTS BY FAMILY_ROOT_FIRST_RANK
# -----------------------
# Lower family_root_first_rank is better. We rank experiments by the mean
# family_root_first_rank across all evaluated rows, pushing experiments with
# no valid family_root hits to the bottom.
...
top_5_detail_df = (
    detail_df[detail_df["experiment"].isin(top_5_experiments)]
    .copy()
)

top_5_detail_df["experiment_rank_by_family_root_first_rank"] = pd.Categorical(
    top_5_detail_df["experiment"],
    categories=top_5_experiments,
    ordered=True,
).codes + 1

top_5_detail_df = top_5_detail_df.merge(
    experiment_family_root_ranking,
    on="experiment",
    how="left",
)
...
top_5_detail_df = top_5_detail_df.sort_values(
    by=[
        ...
        "family_root_first_rank_mean",
        "family_root_first_rank_median",
        "sample",
        "paper",
        "title",
    ]
