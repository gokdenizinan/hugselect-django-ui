...
RelaxLevel = Literal[0, 1, 2, 3, 4]  # 0 high-boost strict, 1 high-boost synonyms, 2 medium-boost grams, 3 low-boost synonyms, 4 dropped
...
def _wrap_boosted_query(query_clause: Dict[str, Any], boost: float) -> Dict[str, Any]:
    return {"bool": {"must": [query_clause], "boost": float(boost)}}
...
class ESQueryBuilderAdaptive:
    ...
    Implements:
      ...
      3) All retrieval logic is should-based; important features get higher boosts instead of hard filters
      4) First relaxed check: synonyms (strict is canonical + grams)
      5) Later stages downshift boost fidelity: grams first, then synonyms, then drop
        ...
        *,
        ...
        if mode in ("best", "canonical_only"):
            value_queries[value_key].append(
                _wrap_boosted_query(_terms_many(fg.fields, [val], k=1), feat_weight)
            )
            for text_clause, factor in self._text_query_clauses_for_value(self._semantic_text_fields(fg), val):
                value_queries[value_key].append(
                    _wrap_boosted_query(text_clause, feat_weight * float(factor))
                )
        ...
        if mode in ("best", "grams_only"):
            ...
            if grams_list:
                value_queries[value_key].append(
                    _wrap_boosted_query(_terms_many(fg.fields, grams_list, k=1), feat_weight * float(grams_factor if grams_factor is not None else self.grams_factor))
                )
        ...
        if mode in ("best", "synonyms_only"):
            ...
            for syn, sim in syn_list[: self.max_synonyms_per_value]:
                value_queries[value_key].append(
                    _wrap_boosted_query(_terms_many(fg.fields, [syn], k=1), feat_weight * float(sim))
                )
    ...
    def display_score_for_hit(
        ...
        *,
        ...
        for fg in groups:
            ...
            mode = self._match_mode_for_group(fg)
    ...
    def build_tier_filter(self, groups: List[FeatureGroup]) -> Dict[str, Any]:
        """
        Build a query clause representing the highest-confidence matching tier.
        This is used only as an additional boosted should-clause, not as a hard
        eligibility gate.
        """
        ...
        for fg in groups:
            ...
            mode = self._match_mode_for_group(fg)
            tier_weight = self._priority_weight(fg.priority, fg.base_weight) * self.LEVEL_PENALTY.get(int(fg.level), 0.0)
            if tier_weight <= 0:
                continue
            ...
            for val in fg.include:
                ...
                if mode in ("best", "canonical_only"):
                    local_candidates.append(_terms_many(fg.fields, [val], k=1))
                    for text_clause, _ in self._text_query_clauses_for_value(self._semantic_text_fields(fg), val):
                        local_candidates.append(text_clause)
                if mode in ("best", "grams_only"):
                    grams = fg.grams_by_value.get(val, [])
                    if grams:
                        local_candidates.append(_terms_many(fg.fields, grams, k=1))
                if mode in ("best", "synonyms_only"):
                    syns = [s for s, _ in fg.syn_by_value.get(val, [])[: self.max_synonyms_per_value]]
                    if syns:
                        local_candidates.append(_terms_many(fg.fields, syns, k=1))
                if local_candidates:
                    value_queries.append(_dis_max_once(local_candidates))

            if value_queries:
                should.append({
                    "bool": {
                        "should": value_queries,
                        "minimum_should_match": 1,
                        "boost": float(tier_weight),
                    }
                })
    ...
    def build_query(self, groups: List[FeatureGroup], *, include_explain: bool = False,
        ...
        Uses:
          - only should-based retrieval/scoring clauses for positive matching
          - must_not for negative exclusions
          - stronger boosts for important features instead of hard filters
        ...
        should.extend(self._global_value_shoulds(groups))
    ...
    def _best_match_for_value_mode(
        ...
        *,
        ...
        if mode in ("best", "canonical_only"):
            ...
            if _cross_field_token_match(source, self._semantic_text_fields(fg), user_value):
                best = max(best, float(feat_weight) * float(self.cross_fields_factor))
            if _phrase_match_in_fields(source, self._semantic_text_fields(fg), user_value):
                best = max(best, float(feat_weight) * float(self.phrase_factor))
    ...
    def _base_constraints_query(self, groups: List[FeatureGroup]) -> Dict[str, Any]:
        """
        Base query used during feasibility checks.
        Since retrieval is fully should-based, this keeps only negative exclusions
        as hard constraints and treats important fixed features as additional
        should-boosts.
        """
        should: List[Dict[str, Any]] = []
        must_not: List[Dict[str, Any]] = []
            ...
            if not fg.relaxable and fg.priority in ("must", "strong_prefer"):
                ...
                should.append({
                    "bool": {
                        "should": variants,
                        "minimum_should_match": 1,
                        "boost": float(self._priority_weight(fg.priority, fg.base_weight)),
                    }
                })
    ...
    def feasibility_counts(
        ...
        def add_count(feature_key: str, label: str, extra_should: Dict[str, Any]):
            keys.append((feature_key, label))
            base_bool = dict((base_q.get("bool") or {}))
            shoulds = list(base_bool.get("should", []))
            shoulds.append(extra_should)
            q_bool: Dict[str, Any] = {"should": shoulds, "minimum_should_match": 1}
            if "must_not" in base_bool:
                q_bool["must_not"] = base_bool["must_not"]
    ...
    def pick_next_relaxation(
        ...
        def sort_key(fg: FeatureGroup):
            # Soften semantically rich must-clauses earlier; keep highly structured filters stricter for longer.
            pr_rank = 0 if fg.priority == "must" else 1
            relax_bucket, _ = self._relaxation_priority_bucket(fg)
            return (pr_rank, relax_bucket, fg.base_weight)
    ...
    def diagnose_expected_model(
        ...
        *,
        ...
        """
        Run a query and produce a diagnostic JSON explaining whether the
        expected model was retrieved and why / why not.
        ...
        model_json_path : str
            Path like: 8-CRITERIA_SELECTION/test_models/google__mt5-base.json
        output_path : optional path to write result json
        ...
        expected_id = normalize_es_id(os.path.basename(model_json_path))

        # Keep a fresh copy of the original feature intent for diagnosis.
        # self.search() mutates group relaxation levels in-place, which can drop
        # all feature groups by the time the search loop finishes. Diagnosis must
        # therefore evaluate the expected model against a newly built, unrelaxed
        # set of groups rather than the post-search groups.
        diagnostic_groups = self.build_feature_groups(features)
        diagnostic_fixed_max_score = self.compute_max_score(diagnostic_groups)

        resp, query, groups = self.search(
            es_client,
            index,
            features,
        ...
        for i, h in enumerate(hits):
            pretty_id = h.get("pretty_id")
            if not pretty_id:
                raw_id = h.get("_id")
                if raw_id:
                    pretty_id = normalize_es_id(raw_id)

            if pretty_id == expected_id:
                found = True
                rank = i + 1
                expected_hit = h
                break
        ...
        result = {
            "expected_model": expected_id,
            "retrieved": found,
            "rank": rank,
            "score": None,
            "display_score": None,
            "score_breakdown": None,
            "expected_model_evaluation": None,
            "query_used": query,
            "top_results": [
                {
                    "rank": i + 1,
                    "model": h.get("pretty_id") or normalize_es_id(h.get("_id")),
                    "score": h.get("_score"),
                    "display_score": h.get("display_score"),
                }
        ...
        try:
            ...
            fake_hit = {
                "_id": expected_id,
                "pretty_id": expected_id,
                "_source": target_doc,
                "_score": 0,
            }

            expected_model_display_score = self.display_score_for_hit(
                diagnostic_groups,
                fake_hit,
                fixed_max_score=diagnostic_fixed_max_score,
            )

            expected_model_breakdown = self.score_breakdown_for_hit(
                diagnostic_groups,
                fake_hit,
                fixed_max_score=diagnostic_fixed_max_score,
            )

            result["expected_model_evaluation"] = {
                "model": expected_id,
                "retrieved": found,
                "rank": rank,
                "feature_matches_evaluated_from_expected_model": True,
                "diagnostic_groups_count": len(diagnostic_groups),
                "search_groups_count": len(groups),
                "score": expected_model_breakdown.get("estimated_raw_score"),
                "display_score": expected_model_display_score,
                "score_breakdown": expected_model_breakdown,
            }
        ...
        if found:
            result["score"] = expected_hit.get("_score")
            result["display_score"] = expected_hit.get("display_score")
            result["score_breakdown"] = expected_hit.get("_score_breakdown")
        elif result.get("expected_model_evaluation") is not None:
            result["score"] = result["expected_model_evaluation"]["score"]
            result["display_score"] = result["expected_model_evaluation"]["display_score"]
            result["score_breakdown"] = result["expected_model_evaluation"]["score_breakdown"]
    ...
    def _pick_last_resort(self, groups: List[FeatureGroup]) -> Optional[FeatureGroup]:
        ...
        def sort_key(fg: FeatureGroup):
            pr_rank = 0 if fg.priority == "must" else 1
            relax_bucket, _ = self._relaxation_priority_bucket(fg)
            return (pr_rank, relax_bucket, fg.base_weight, fg.level)
