...
class ESQueryBuilderAdaptive:
    ...
    def __init__(
        ...
        *,
        ...
        if synonym_cache is not None:
            ...
            self.synonym_cache = SimpleSynonymCache(synonym_cache_path)
        else:
    ...
    def _grams_of_synonyms_by_value(self, fg: FeatureGroup, user_value: Any) -> List[str]:
        return self._grams_of_synonyms_cached(fg=fg, user_value=user_value)

    def _matched_term_key(self, term: Any) -> str:
        norm = _normalize_text(term)
        return norm or self._value_key(term)

    def _candidate_term_details_for_value(
        self,
        fg: FeatureGroup,
        user_value: Any,
        feat_weight: float,
        ...
        """
        Return candidate queries grouped by the normalized matched term they target.
        This lets the builder collapse duplicate score contributions when distinct
        user values or feature groups ultimately match the same underlying term.
        """
        if feat_weight <= 0:
            return []
        ...
        if self._is_quality_feature(fg.feature_key):
            ...
            if quality_filter is None:
                return []
            term_key = f"quality::{fg.feature_key}"
            score = feat_weight * float(self.MATCH_TYPE_BOOSTS["canonical"])
            return [(
                term_key,
                _wrap_constant_score(quality_filter, score),
                {
                    "user_value": user_value,
                    "feature_key": fg.feature_key,
                    "priority": fg.priority,
                    "effective_feat_weight": feat_weight,
                    "matched_term": fg.feature_key,
                    "match_type": "quality_threshold",
        ...
        def _append(term: Any, query: Dict[str, Any], match_type: str, boost_factor: float) -> None:
            term_key = self._matched_term_key(term)
        ...
        _append(
            ...
            _wrap_constant_score(
                _terms_many(fg.fields, [user_value], k=1),
                float(feat_weight) * float(self.MATCH_TYPE_BOOSTS["canonical"]),
            ),
            "canonical",
        ...
        for gram in fg.grams_by_value.get(user_value, []):
            _append(
                ...
                _wrap_constant_score(
                    ...
                    float(feat_weight) * float(self.MATCH_TYPE_BOOSTS["grams"]),
                ),
                "grams",
                float(self.MATCH_TYPE_BOOSTS["grams"]),
            )
        ...
        for syn, _ in fg.syn_by_value.get(user_value, [])[: self.max_synonyms_per_value]:
            _append(
                ...
                _wrap_constant_score(
                    ...
                    float(feat_weight) * float(self.MATCH_TYPE_BOOSTS["synonyms"]),
                ),
                "synonyms",
        ...
        for syn_gram in self._grams_of_synonyms_by_value(fg, user_value):
            _append(
                syn_gram,
                _wrap_constant_score(
                    _terms_many(fg.fields, [syn_gram], k=1),
                    float(feat_weight) * float(self.MATCH_TYPE_BOOSTS["grams_of_synonyms"]),
                ),
                "grams_of_synonyms",
                float(self.MATCH_TYPE_BOOSTS["grams_of_synonyms"]),
            )
    ...
    def _global_value_shoulds(self, groups: List[FeatureGroup]) -> List[Dict[str, Any]]:
        """
        Build ONE dis_max per normalized matched term across ALL features/values.
        This prevents the same underlying matched term from contributing more than
        once even when it is reachable from multiple user values or feature groups.
        """
        term_queries: Dict[str, List[Dict[str, Any]]] = defaultdict(list)
        ...
        for fg in groups:
            ...
            for val in fg.include:
                for term_key, query, _meta in self._candidate_term_details_for_value(
                    fg,
                    val,
                    float(feat_weight),
                ):
                    term_queries[term_key].append(query)

        shoulds: List[Dict[str, Any]] = []
        for _, queries in term_queries.items():
            if queries:
    ...
    def display_score_for_hit(
        ...
        *,
        ...
        best_by_term: Dict[str, float] = defaultdict(float)
        ...
        for fg in groups:
            ...
            for user_value in fg.include:
                ...
                term_key = self._matched_term_key(detail.get("matched_term") or user_value)
                if score > best_by_term[term_key]:
                    best_by_term[term_key] = score

        feature_total = float(sum(best_by_term.values()))
    ...
    def _feature_total_global_by_value(
        ...
        best_score_by_term: Dict[str, float] = defaultdict(float)
        ...
        for fg in groups:
            ...
            for user_value in fg.include:
                score, detail = self._best_match_for_value_detail(fg, source, user_value, feat_weight)
                term_key = self._matched_term_key(detail.get("matched_term") or user_value)
                if score > best_score_by_term[term_key]:
                    best_score_by_term[term_key] = score
                    winners[term_key] = {
                        "term_key": term_key,
                        "user_value": user_value,
                        "feature_key": fg.feature_key,
        ...
        total = float(sum(best_score_by_term.values()))
    ...
    def score_breakdown_for_hit(
        ...
        *,
        ...
        # Global feature score mirrors ES dis_max-by-matched-term behavior.
        ...
        for fg in groups:
            ...
            if fg.feature_key == "last_modified_year":
                ...
                feature_rows.append({
                    ...
                    "per_value_dismax": [{
                        ...
                        "global_term_winner": False,
                        "global_term_contribution": 0.0,
                    }],
            ...
            for user_value in fg.include:
                ...
                winner = winners.get(self._matched_term_key(best_detail.get("matched_term") or user_value))
                winner_for_this_feature = bool(
                    winner
                    and winner.get("feature_key") == fg.feature_key
                    and self._value_key(winner.get("user_value")) == self._value_key(user_value)
                    and self._matched_term_key(winner.get("matched_term")) == self._matched_term_key(best_detail.get("matched_term") or user_value)
                )
                ...
                per_value.append({
                    "user_value": user_value,
                    "best": best_detail,
                    "global_term_winner": winner_for_this_feature,
                    "global_term_contribution": global_score,
                })
        ...
        return {
            ...
            "notes": {
                "estimation": (
                    ...
                    "global dis_max-by-matched-term feature score used for normalization. "
    ...
    def compute_max_score(self, groups: List[FeatureGroup]) -> float:
        ...
        for fg in groups:
            ...
            if self._is_quality_feature(fg.feature_key):
                term_key = f"quality::{fg.feature_key}"
                if term_key not in best_by_term or base > best_by_term[term_key]:
                    best_by_term[term_key] = float(base)
                continue
            for val in fg.include:
                for term_key, _query, meta in self._candidate_term_details_for_value(fg, val, float(base)):
                    score = float(meta.get("score", 0.0))
                    if term_key not in best_by_term or score > best_by_term[term_key]:
                        best_by_term[term_key] = score
        return float(sum(best_by_term.values()))
    ...
    def compare_bundle_to_sample(
        ...
        matched_items = []
        for _, item in best_by_term.items():
    ...
    def precompute_feature_group_cache(self, features: Any) -> List[FeatureGroup]:
        ...
        for fg in groups:
            for user_value in fg.include:
                self._grams_of_synonyms_by_value(fg, user_value)
        return groups
