...
class ESQueryBuilderAdaptive:
    ...
    def _add_value_candidates(
        ...
        *,
        ...
        for syn_gram in self._grams_of_synonyms_by_value(fg, val):
            self._add_match_bucket_candidate(
                bucket_queries=bucket_queries,
                bucket_key=self._match_bucket_key(syn_gram),
                clause=_wrap_constant_score(
    ...
    def compute_max_score(self, groups: List[FeatureGroup]) -> float:
        ...
        for fg in groups:
            ...
            for val in fg.include:
                ...
                candidates.extend((syn_gram, float(self.MATCH_TYPE_BOOSTS["grams_of_synonyms"])) for syn_gram in self._grams_of_synonyms_by_value(fg, val))
