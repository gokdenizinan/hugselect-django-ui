...
OUT_COMPACT_GROUPED_CSV = os.path.join(OUT_DIR, "experiment_sample_stats_compact_grouped.csv")
...
def get_family_root_for_model(model_id, model_meta):
    attrs = get_model_attrs(model_id, model_meta)
    return normalize_attr_value(attrs.get("family_root"))


def normalize_family_root_list(recs, model_meta, k=10):
    if not isinstance(recs, list):
        return []

    out = []
    for rec in recs[:k]:
        fam = get_family_root_for_model(rec, model_meta)
        if fam is None:
            continue
        if isinstance(fam, list):
            out.extend([str(x).strip().lower() for x in fam if x is not None])
        else:
            out.append(str(fam).strip().lower())
    return out


def overlap_ratio_family_root_against_reference(reference_recs, other_recs, model_meta, k=10):
    ref_topk = normalize_family_root_list(reference_recs, model_meta=model_meta, k=k)
    other_topk = normalize_family_root_list(other_recs, model_meta=model_meta, k=k)
...
def build_method_total_summary(merged_df, detail_df, all_sample_norms, model_meta, k=10):
    rows = []
    ...
    for source_name in source_names:
        ...
        joined[f"family_root_overlap_with_recsys@{k}"] = joined.apply(
            lambda row: overlap_ratio_family_root_against_reference(
                row["recsys_recommendations"],
                row["method_recommendations"],
        ...
        joined[f"family_root_overlap_with_recsys@{k}"] = joined[f"family_root_overlap_with_recsys@{k}"].fillna(0.0)
        ...
        rows.append({
            ...
            f"model_id_coverage@{k}": round(float(joined[f"hit@{k}"].mean()), 4),
            ...
            f"model_id_overlap_with_recsys@{k}": round(float(joined[f"overlap_with_recsys@{k}"].mean()), 4),
            f"family_root_overlap_with_recsys@{k}": round(float(joined[f"family_root_overlap_with_recsys@{k}"].mean()), 4),
...
# -----------------------
all_sample_norms = sorted(df["sample_norm"].dropna().unique().tolist())
...
OUT_RECOMMENDATIONS_GT_CSV = os.path.join(
    OUT_DIR,
    "ground_truth_vs_recommendations_grouped.csv"
)


def value_at_rank(recs, rank_1_based):
    if not isinstance(recs, list):
        return ""
    idx = rank_1_based - 1
...
def attrs_for_display(model_id, model_meta):
    attrs = get_model_attrs(model_id, model_meta)
    return {
        "family_root": display_attr_value(normalize_attr_value(attrs.get("family_root")), missing_value=""),
        "modality": display_attr_value(normalize_attr_value(attrs.get("assigned_modality")), missing_value=""),
        "task": display_attr_value(normalize_attr_value(attrs.get("task")), missing_value=""),
    }


# Use best recsys experiment + one row per sample for each baseline
source_recs = {}

if best_experiment is not None:
    source_recs["recsys"] = (
        merged[
            (merged["source"] == "recsys") &
            (merged["experiment"] == best_experiment)
        ]
        .sort_values(by=["sample_norm"])
        .drop_duplicates(subset=["sample_norm"], keep="first")
        [["sample_norm", "recommendations"]]
    )

for source_name in ["chatgpt", "gemini", "claude", "perplexity"]:
    tmp = merged[merged["source"] == source_name].copy()
    tmp = select_one_row_per_sample_for_source(tmp)
    source_recs[source_name] = (
        tmp.sort_values(by=["sample_norm"])
        .drop_duplicates(subset=["sample_norm"], keep="first")
        [["sample_norm", "recommendations"]]
    )
...
gt_base = gt_base.sort_values(
    by="sample",
    key=sample_sort_key
).reset_index(drop=True)

# Attach recommendation lists
recommendation_wide = gt_base.copy()

for source_name, src in source_recs.items():
    recommendation_wide = recommendation_wide.merge(
        src.rename(columns={"recommendations": f"{source_name}_recommendations"}),
        on="sample_norm",
        how="left",
    )
...
for _, row in recommendation_wide.iterrows():
    gt_model_id = row["modelID"]
    gt_attrs = attrs_for_display(gt_model_id, model_meta)

    for rank_i in range(1, K + 1):
        out = {
            ("metadata", "sample"): row["sample"],
            ("metadata", "paper"): row["paper"],
            ("metadata", "title"): row["title"],
            ("metadata", "year"): row["year"],
            ("metadata", "venue"): row["venue"],
            ("metadata", "recommendation_rank"): rank_i,
        }

        # Only show ground truth on first recommendation row per sample
        show_gt = rank_i == 1

        out[("model_id", "ground_truth")] = gt_model_id if show_gt else ""
        out[("family_root", "ground_truth")] = gt_attrs["family_root"] if show_gt else ""
        out[("modality", "ground_truth")] = gt_attrs["modality"] if show_gt else ""
        out[("task", "ground_truth")] = gt_attrs["task"] if show_gt else ""

        for source_name in ["recsys", "chatgpt", "gemini", "claude", "perplexity"]:
            recs = row.get(f"{source_name}_recommendations", [])
            rec_model_id = value_at_rank(recs, rank_i)
            rec_attrs = attrs_for_display(rec_model_id, model_meta) if rec_model_id else {
                "family_root": "",
                "modality": "",
                "task": "",
            }

            out[("model_id", source_name)] = rec_model_id
            out[("family_root", source_name)] = rec_attrs["family_root"]
            out[("modality", source_name)] = rec_attrs["modality"]
            out[("task", source_name)] = rec_attrs["task"]

        rows.append(out)

gt_recommendations_grouped_df = pd.DataFrame(rows)
gt_recommendations_grouped_df.columns = pd.MultiIndex.from_tuples(
    gt_recommendations_grouped_df.columns
)
...
ordered_cols = [
    ...
    ("metadata", "year"),
    ("metadata", "venue"),
    ("metadata", "recommendation_rank"),
]

for group in ["model_id", "family_root", "modality", "task"]:
    ordered_cols.extend([
        (group, "ground_truth"),
        (group, "recsys"),
        (group, "chatgpt"),
...
gt_recommendations_grouped_df = gt_recommendations_grouped_df[
    ordered_cols
]

# -----------------------
# 11.6) FAMILY-LEVEL OVERALL METRICS (AGGREGATED)
# -----------------------
OUT_FAMILY_EVAL_SUMMARY_CSV = os.path.join(
    OUT_DIR,
    "family_level_evaluation_summary.csv"
)

def family_level_metrics_at_k(gt_model_id, recs, model_meta, k=10):
    gt_families = set(normalize_family_root_list([gt_model_id], model_meta, k=1))
    if not gt_families:
        return np.nan, np.nan, np.nan

    recs_topk = recs[:k] if isinstance(recs, list) else []

    relevance = []
    retrieved = set()

    for rec in recs_topk:
        rec_fams = set(normalize_family_root_list([rec], model_meta, k=1))
        is_rel = len(gt_families & rec_fams) > 0
        relevance.append(1 if is_rel else 0)
        if is_rel:
            retrieved |= (gt_families & rec_fams)

    relevance += [0] * (k - len(relevance))

    precision = sum(relevance[:k]) / k
    recall = len(retrieved) / len(gt_families)

    dcg = sum(rel / math.log2(i + 2) for i, rel in enumerate(relevance[:k]))
    ideal = [1] * min(len(gt_families), k) + [0] * (k - min(len(gt_families), k))
...
for source_name in ["recsys", "chatgpt", "gemini", "claude", "perplexity"]:
    ...
    for _, row in recommendation_wide.iterrows():
        gt_model = row["modelID"]
        recs = row.get(f"{source_name}_recommendations", [])
        hugselect_recs = row.get("recsys_recommendations", [])

        p, r, n = family_level_metrics_at_k(gt_model, recs, model_meta, k=K)
        ...
        if source_name != "recsys":
            overlaps_model.append(
                overlap_ratio_against_reference(hugselect_recs, recs, k=K)
            )
            overlaps_family.append(
                overlap_ratio_family_root_against_reference(
                    hugselect_recs, recs, model_meta, k=K
                )
            )

    summary_rows.append({
        "source": source_name,
        f"family_precision@{K}": round(float(np.mean(precisions)), 4) if precisions else np.nan,
        f"family_recall@{K}": round(float(np.mean(recalls)), 4) if recalls else np.nan,
        f"family_ndcg@{K}": round(float(np.mean(ndcgs)), 4) if ndcgs else np.nan,
        f"model_overlap_with_hugselect@{K}": (
            1.0 if source_name == "recsys"
            else round(float(np.nanmean(overlaps_model)), 4)
        ),
        f"family_overlap_with_hugselect@{K}": (
            1.0 if source_name == "recsys"
            else round(float(np.nanmean(overlaps_family)), 4)
        ),
        "num_samples": len(recommendation_wide),
    })

family_eval_summary_df = pd.DataFrame(summary_rows)
family_eval_summary_df = family_eval_summary_df.sort_values(by="source")

family_eval_summary_df.to_csv(
    OUT_FAMILY_EVAL_SUMMARY_CSV,
    index=False,
    encoding="utf-8"
)

print(f"Saved FAMILY-LEVEL SUMMARY CSV to: {OUT_FAMILY_EVAL_SUMMARY_CSV}")
# -----------------------
# 12) SAVE
# -----------------------
os.makedirs(OUT_DIR, exist_ok=True)
...
# Columns that should stay as regular single columns
base_columns = {
    "sample",
    "paper",
    "title",
...
# Group patterns: suffix -> top-level group name
group_suffixes = {
    "num_recommendations": "num_recommendations",
    "rank": "rank",
    "family_root_first_rank": "family_root_first_rank",
    "family_root_match_rate": "family_root_match_rate",
    "assigned_modality_first_rank": "assigned_modality_first_rank",
    "assigned_modality_match_rate": "assigned_modality_match_rate",
    "task_first_rank": "task_first_rank",
...
for col in df.columns:
    if col in base_columns:
        new_columns.append(("metadata", col))
        continue

    matched = False
    for suffix, group_name in group_suffixes.items():
        for system in systems:
            expected = f"{system}_{suffix}"
            if col == expected:
                new_columns.append((group_name, system))
                matched = True
                break
        if matched:
            break
...
# Apply hierarchical columns
df.columns = pd.MultiIndex.from_tuples(new_columns)

df.to_csv(OUT_COMPACT_GROUPED_CSV, index=False, encoding="utf-8")

gt_recommendations_grouped_df.to_csv(
    OUT_RECOMMENDATIONS_GT_CSV,
    index=False,
    encoding="utf-8"
)

print(f"Saved ground truth vs recommendations CSV to: {OUT_RECOMMENDATIONS_GT_CSV}")