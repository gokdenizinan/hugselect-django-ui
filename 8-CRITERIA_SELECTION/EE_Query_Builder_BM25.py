...
class ESQueryBuilderAdaptive:
    """
    Pure-BM25-oriented Elasticsearch query builder.

    Differences from the current version:
    - Uses multi_match / match queries instead of constant_score(term filters)
    - Does NOT use function_score rank functions
    - Keeps hard filters like last_modified_year
    - Keeps feature-group construction, grams, synonyms, and dis_max
    - Lets Elasticsearch BM25 control ranking via _score
    """
    ...
    def __init__(
        ...
        mapping,
        *,
        base_signal_weights=None,
        priority_multipliers=None,
        feature_weight_groups=None,
        boost_config=None,
        size: int = 5,
        target_hits: int = 20,
        ...
        enable_rank_functions: bool = False,   # forced off for pure BM25
        ...
        # Pure BM25: turn these off in practice
        self.enable_rank_functions = False
        ...
        self.rank_signal_specs = []     # no rank signals in pure BM25
        self.rank_functions = []        # no function_score in pure BM25
    ...
    def _priority_weight(self, priority, base: float) -> float:
    ...
    def _build_quality_feature_specs(self):
    ...
    def _quality_threshold_filter(self, fg) -> Optional[Dict[str, Any]]:
    ...
    def _safe_pref_priority(self, pref: Any, default="prefer"):
    ...
    def _last_modified_cutoff_year(self, fg) -> Optional[int]:
        if fg.feature_key != "last_modified_year" or not fg.include:
            return None
        return _coerce_year(fg.include[0])

    def _last_modified_filter_clause(self, fg) -> Optional[Dict[str, Any]]:
    ...
    def _doc_satisfies_last_modified(self, source: Dict[str, Any], fg) -> bool:
        cutoff_year = self._last_modified_cutoff_year(fg)
        if cutoff_year is None or not fg.fields:
            return True
    ...
    def _make_categorical_group(
        ...
        *,
        ...
        pr = force_priority or getattr(pref, "priority", "prefer")
        include = _dedupe_values_preserve_order(list(getattr(pref, "include", None) or []))
        exclude = _dedupe_values_preserve_order(list(getattr(pref, "exclude", None) or []))
    ...
    def build_feature_groups(self, features: Any):
        ...
        for key, field_key in [
            ...
            if pref and (getattr(pref, "include", None) or getattr(pref, "exclude", None)):
                ...
                if (fg.include or fg.exclude) and fg.fields:
                    groups.append(fg)
    ...
    # ----------------------------
    # Pure BM25 query helpers
    # ----------------------------

    def _bm25_clause_for_value(
        self,
        *,
        fields: List[str],
        value: Any,
        ...
        query_type = "phrase" if phrase else "best_fields"

        return {
            "multi_match": {
                "query": text,
                "fields": [f"{field}^{float(boost)}" for field in fields],
                "type": query_type,
            }
        }

    def _add_value_candidates_bm25(
        self,
        *,
        value_key: str,
        ...
        if self._is_quality_feature(fg.feature_key):
            ...
            if quality_filter is not None:
                value_queries[value_key].append({"bool": {"filter": [quality_filter]}})
            return

        canonical_boost = feat_weight * float(self.MATCH_TYPE_BOOSTS["canonical"])
        clause = self._bm25_clause_for_value(
            fields=fg.fields,
            value=val,
        ...
        if clause:
            value_queries[value_key].append(clause)

        for gram in fg.grams_by_value.get(val, []):
            gram_clause = self._bm25_clause_for_value(
                fields=fg.fields,
                value=gram,
                boost=feat_weight * float(self.grams_factor),
                phrase=False,
            )
            if gram_clause:
                value_queries[value_key].append(gram_clause)

        for syn, confidence in fg.syn_by_value.get(val, [])[: self.max_synonyms_per_value]:
            synonym_boost = feat_weight * float(self.MATCH_TYPE_BOOSTS["synonyms"]) * float(confidence)
            if synonym_boost <= 0:
                continue
            syn_clause = self._bm25_clause_for_value(
                fields=fg.fields,
                value=syn,
                boost=synonym_boost,
                phrase=False,
            ...
            if syn_clause:
                value_queries[value_key].append(syn_clause)

    def _global_value_shoulds_bm25(self, groups: List[FeatureGroup]) -> List[Dict[str, Any]]:
        ...
        for fg in groups:
            ...
            for val in fg.include:
                ...
                self._add_value_candidates_bm25(
                    value_key=value_key,
    ...
    def build_query(self, groups: List[FeatureGroup], *, include_explain: bool = False) -> Dict[str, Any]:
        """
        Pure-BM25 query:
        - text relevance from multi_match clauses
        - hard filters only for real constraints
        - no function_score
        - no constant_score
        """
        should = self._global_value_shoulds_bm25(groups)
        ...
        for fg in groups:
            ...
            if fg.exclude and fg.fields and not self._is_quality_feature(fg.feature_key):
                for value in fg.exclude:
                    excl = self._bm25_clause_for_value(
                        fields=fg.fields,
                        value=value,
                        boost=1.0,
                        phrase=False,
                    ...
                    if excl:
                        must_not_clauses.append(excl)

        bool_query: Dict[str, Any] = {"bool": {}}

        if should:
            bool_query["bool"]["should"] = should
            bool_query["bool"]["minimum_should_match"] = int(max(1, self.minimum_should_match))
        ...
        if not should and not filter_clauses and not must_not_clauses:
            base_query: Dict[str, Any] = {"match_all": {}}
        else:
            base_query = bool_query
    ...
    def search(
        ...
        *,
        ...
        prebuilt_groups: Optional[List[FeatureGroup]] = None,
    ):
