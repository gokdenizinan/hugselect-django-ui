...
from EE_Query_Builder_Pure_BM25_no_synonyms import ESQueryBuilderAdaptive
from EE_Query_Builder_Pure_BM25_no_synonyms import query_mapping
...
OUTPUT_LETTER = "BM2" # yeniler gden basliyor
...
HYBRID_SEARCH_ENABLED = False  # Pure BM25 variant keeps vector search disabled.
...
# Feature matching is intentionally unweighted. Rank metadata is used only
# as a secondary tie-break after equal BM25 scores.
BASE_BOOST_CONFIG = {
    "match_mode": {
        "grams_factor": 0.90,
    },
...
def make_experiment_config(exp_id: int, combo_dict: Dict[str, Any]) -> ExperimentConfig:
    boost_config = deepcopy(BASE_BOOST_CONFIG)
...
def run_pure_bm25_search(
    ...
    search_stage_timings["builder_search_pure_bm25_seconds"] = time.perf_counter() - t0

    search_query = {"bm25": final_query}

    if not exp_cfg.hybrid_search_enabled or exp_cfg.vector_rrf_weight <= 0:
        search_stage_timings["hybrid_total_seconds"] = 0.0
        return bm25_response, search_query, final_feature_groups, search_stage_timings, builder, prebuilt_groups
    ...
    if query_vector is None:
        search_query["vector_skipped"] = "encoder_unavailable"
    ...
    search_query["vector"] = vector_query
    ...
    try:
        ...
        search_query["vector_error"] = str(e)
        search_stage_timings["hybrid_total_seconds"] = time.perf_counter() - hybrid_started
        return bm25_response, search_query, final_feature_groups, search_stage_timings, builder, prebuilt_groups
...
def run_single_experiment(
    ...
    t0 = time.perf_counter()
    response, final_query, final_feature_groups, search_stage_timings, builder, prebuilt_groups = run_pure_bm25_search(
        exp_cfg=exp_cfg,
        features=bundle,
        user_text=user_text,
