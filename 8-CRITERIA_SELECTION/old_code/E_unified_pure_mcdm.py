...
OUTPUT_LETTER = "MCDM" # yeniler gden basliyor
# =========================================================
# SEARCH MODE
# =========================================================
# "hybrid"      -> current behavior
# "pure_bm25"   -> lexical BM25-only
# "pure_mcdm"   -> fetch broad candidate pool, rank only with MCDM score
SEARCH_MODE = "pure_mcdm"

# how many docs to fetch before MCDM ranking
PURE_MCDM_CANDIDATE_POOL = 1000
...
def build_mcdm_candidate_query(
    builder,
    prebuilt_groups,
    *,
    ...
    """
    Broad candidate fetch for pure MCDM:
      - keeps only hard filters (e.g. last_modified_year)
      - does NOT use BM25 text scoring
      - does NOT use function_score / rank functions
      - sorting is deterministic but not relevance-based
    """
    base_constraints = builder._base_constraints_query(prebuilt_groups)
    filter_clauses = (base_constraints.get("bool", {}) or {}).get("filter", []) or []

    query: Dict[str, Any] = {
        "explain": bool(include_explain),
        "size": int(candidate_pool_size),
        "_source": {"includes": builder._collect_needed_source_paths(prebuilt_groups)},
        "query": {
            "bool": {
...
def rerank_hits_with_mcdm(
    ...
    *,
    ...
    """
    Score every candidate only with the builder's MCDM-style feature scorer,
    then return an ES-like response object with the same shape as before.
    """
    fixed_max_score = builder.compute_max_score(prebuilt_groups)
    reranked_hits = []

    for hit in hits:
        source = hit.get("_source", {}) or {}

        # raw feature-total winner computation
        raw_total, winners = builder._feature_total_global_by_value(prebuilt_groups, source)

        # normalized display score
        normalized_score = builder.display_score_for_hit(
            prebuilt_groups,
            hit,
            fixed_max_score=fixed_max_score,
        )

        new_hit = deepcopy(hit)
        new_hit["_score"] = round2(normalized_score)
        new_hit["_mcdm"] = {
            "raw_total": round2(raw_total),
            "normalized_score": round2(normalized_score),
            "fixed_max_score": round2(fixed_max_score),
    ...
    reranked_hits.sort(
        key=lambda h: (
            -(h.get("_score") or 0.0),
            -((h.get("_mcdm") or {}).get("raw_total") or 0.0),
            str((h.get("_source", {}) or {}).get("modelID")
                or (h.get("_source", {}) or {}).get("model_id")
                or h.get("_id")
                or ""),
        )
    )

    reranked_hits = reranked_hits[:size]
...
def run_pure_mcdm_search(
    ...
    """
    Same return signature as run_bm25_and_optional_vector_search(...),
    so run_single_experiment(...) does not need to change.
    """
    ...
    t0 = time.perf_counter()
    candidate_query = build_mcdm_candidate_query(
        builder=builder,
        prebuilt_groups=prebuilt_groups,
        candidate_pool_size=PURE_MCDM_CANDIDATE_POOL,
        include_explain=INCLUDE_EXPLAIN,
    )
    search_stage_timings["build_mcdm_candidate_query_seconds"] = time.perf_counter() - t0

    t0 = time.perf_counter()
    candidate_response = es_client.search(index=index_name, body=candidate_query)
    search_stage_timings["candidate_fetch_seconds"] = time.perf_counter() - t0

    t0 = time.perf_counter()
    candidate_hits = candidate_response.get("hits", {}).get("hits", [])
    mcdm_response = rerank_hits_with_mcdm(
        builder=builder,
        prebuilt_groups=prebuilt_groups,
        hits=candidate_hits,
        size=exp_cfg.size,
    )
    search_stage_timings["mcdm_rerank_seconds"] = time.perf_counter() - t0

    search_stage_timings["hybrid_total_seconds"] = 0.0

    final_query = {
        "candidate_fetch": candidate_query,
        "ranking_mode": "pure_mcdm",
        "candidate_pool_size": PURE_MCDM_CANDIDATE_POOL,
    }

    return mcdm_response, final_query, prebuilt_groups, search_stage_timings, builder, prebuilt_groups
def run_bm25_and_optional_vector_search(
    ...
    if SEARCH_MODE == "pure_mcdm":
        return run_pure_mcdm_search(
            exp_cfg=exp_cfg,
            features=features,
            user_text=user_text,
    ...
    if SEARCH_MODE == "pure_bm25":
        return run_pure_bm25_search(
            exp_cfg=exp_cfg,
            features=features,
            user_text=user_text,
            index_name=index_name,
    ...
    hybrid_query["vector"] = make_knn_query(
        query_vector=query_vector,
    ...
    try:
        t0 = time.perf_counter()
        vector_response = es_client.search(index=index_name, body=hybrid_query["vector"])
