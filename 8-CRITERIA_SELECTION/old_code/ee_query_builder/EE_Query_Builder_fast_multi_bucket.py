...
class ESQueryBuilderAdaptive:
    ...
    def _resolve_syns_grouped_cached(
        ...
        *,
        ...
        print(f"[TIMER] resolve_grams_grouped: {time.time() - start:.3f}s")
        return self._cache_get_or_compute(
            key,
    ...
    def _matched_buckets_for_value(
        self,
        fg: FeatureGroup,
        source: Dict[str, Any],
        user_value: Any,
        ...
        if self._is_quality_feature(fg.feature_key):
            ...
            if score > 0:
                return [{
                    ...
                    "boost_factor": detail["boost_factor"],
                    "score": score,
                }]
            return []
        ...
        if self._doc_has_term_in_any_field(source, fg.fields, user_value):
            matches.append({
                "bucket_key": self._match_bucket_key(user_value),
        ...
        for match in matches:
            ...
            if current is None or float(match["score"]) > float(current["score"]):
                best_by_bucket[bucket_key] = match
    ...
    def _best_match_for_value_detail(
        ...
        matched_items = self._matched_buckets_for_value(fg, source, user_value, feat_weight)
        ...
        best_item = max(matched_items, key=lambda x: float(x["score"]))
        return float(best_item["score"]), {
            "match_type": best_item["match_type"],
            "matched_term": best_item["matched_term"],
            "boost_factor": best_item["boost_factor"],
        ...
        *,
        ...
        for fg in groups:
            ...
            for user_value in fg.include:
                ...
                feat_sum_local += float(sum(float(item["score"]) for item in matched_items))

                item_rows: List[Dict[str, Any]] = []
                user_global_total = 0.0
                for item in matched_items:
                    ...
                    winner_for_this_feature = bool(
                        ...
                        and winner.get("matched_term") == item.get("matched_term")
                    )
                    global_score = float(winner.get("score", 0.0)) if winner_for_this_feature else 0.0
                    user_global_total += global_score
                    item_rows.append({
                        "bucket_key": bucket_key,
                        "match_type": item.get("match_type"),
                        "matched_term": item.get("matched_term"),
                        "boost_factor": item.get("boost_factor", 0.0),
                        "score": float(item.get("score", 0.0)),
                        "global_value_winner": winner_for_this_feature,
                        "global_value_contribution": global_score,
                    })

                feat_sum_global += user_global_total

                per_value.append({
                    "user_value": user_value,
                    "best": best_detail,
                    "matched_items": item_rows,
                    "global_value_winner": any(item["global_value_winner"] for item in item_rows),
                    "global_value_contribution": user_global_total,
                    "match_bucket_key": best_detail.get("matched_term") and self._match_bucket_key(best_detail.get("matched_term")),
                })
    ...
    def compare_bundle_to_sample(
        ...
        for fg in groups:
            ...
            for user_value in fg.include:
                ...
                if self._is_quality_feature(fg.feature_key):
                    ...
                    bucket_key = self._match_bucket_key(detail.get("matched_term"))
                item["match_bucket_key"] = bucket_key
                if bucket_key:
        ...
        matched_items = []
        for _, item in best_by_bucket.items():
