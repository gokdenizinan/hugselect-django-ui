...
class ESQueryBuilderAdaptive:
    """
    BM25-first Elasticsearch query builder.

    Positive matching uses BM25 query clauses only. Grams and semantic
    synonym expansion are intentionally disabled so the retrieval layer is
    a pure lexical/BM25 search over the indexed fields.

    To prevent the same user value from contributing multiple times when it
    appears through different feature groups or fields, the builder creates
    a single global dis_max winner per normalized user value.
    """
    ...
    def __init__(
        ...
        *,
        ...
        # Feature matching is intentionally unweighted: every matched user
        # value contributes equally regardless of feature group or priority.
        self.feature_weight_groups: Dict[str, Dict[str, float]] = {}
        ...
        # Rank signals are used only as deterministic tie-breakers after BM25 _score.
        self.rank_field_names: List[str] = [spec[0] for spec in self._rank_signal_specs()]
    ...
    def _quality_feature_specs(self) -> List[Tuple[str, str, float]]:
        ...
        for attr_name in quality_attrs:
            ...
            specs.append((attr_name, field_path, 1.0))
        return specs
    ...
    def _rank_signal_specs(self) -> List[Tuple[str, str, Optional[str], float, float]]:
        ...
        # Lexicographic tie-break order: more recent demand first, then broad popularity,
        # then quality dimensions. No human weights are applied.
        specs.append(("downloads_last_30_days", downloads_30d_field, None, 0.0, 1.0))
        specs.append(("likes", likes_field, None, 0.0, 1.0))

        if self.enable_quality_dimensions:
            for attr_name, field_path, _ in self._quality_feature_specs():
                specs.append((attr_name, field_path, None, 0.0, 1.0))
    ...
    def _grams_of_synonyms_cached(
        ...
        *,
        ...
        payload = self._syn_cache_payload(
            op="grams_of_synonyms",
            candidates_sources=candidates_sources,
            values=[user_value],
            feature_key=fg.feature_key,
            extra={"syn_terms": [_normalize_text(s) for s in syn_terms]},
        )
        key = _cache_key("grams_of_synonyms", payload)
        ...
        return self._cache_get_or_compute(key, _compute)
    ...
    def _is_exact_value_field(self, field: str) -> bool:
        exact_suffixes = (
            "model_id",
            "modelID",
            "author",
        ...
        return field in exact_suffixes

    def _make_bm25_value_query(
        self,
        *,
        fields: List[str],
        value: Any,
        ...
        candidate_texts = []
        for item in [text_value, normalized]:
            item = str(item).strip()
            if item and item not in candidate_texts:
                candidate_texts.append(item)

        per_field_queries: List[Dict[str, Any]] = []
        for field in fields:
            if self._is_exact_value_field(field):
                exact_variants = []
                for candidate in _canonical_compare_variants(value) + _lexical_variants(value):
                    candidate = str(candidate).strip()
                    if candidate and candidate not in exact_variants:
                        exact_variants.append(candidate)
                for candidate in exact_variants:
                    per_field_queries.append({"term": {field: {"value": candidate, "boost": float(boost) * 1.15}}})
            for candidate in candidate_texts:
                per_field_queries.append({
                    "match_phrase": {
                        field: {
                            "query": candidate,
                            "boost": float(boost) * 1.10,
                        }
                    }
                })
                per_field_queries.append({
        ...
        if not per_field_queries:
            return {"match_none": {}}
        return _dis_max_once(per_field_queries)
        ...
        if self._is_quality_feature(fg.feature_key):
            ...
            if quality_filter is not None:
                value_queries[value_key].append(
                    _wrap_constant_score(quality_filter, feat_weight * float(self.MATCH_TYPE_BOOSTS["bm25"]))
                )
            return

        value_queries[value_key].append(
            self._make_bm25_value_query(
                fields=fg.fields,
                value=val,
                boost=feat_weight * float(self.MATCH_TYPE_BOOSTS["bm25"]),
            )
        )
    ...
    def _best_match_for_value_detail(
        ...
        if self._doc_has_term_in_any_field(source, fg.fields, user_value):
            score = float(feat_weight) * float(self.MATCH_TYPE_BOOSTS["bm25"])
            return score, {
                "match_type": "bm25_exactish",
                "matched_term": user_value,
                "boost_factor": float(self.MATCH_TYPE_BOOSTS["bm25"]),
                "score": score,
            }
    ...
    def _make_categorical_group(
        ...
        *,
        ...
        # Pure BM25 mode: do not expand grams or semantic synonyms.
    ...
    def _best_match_for_value(
        ...
        *,
        ...
        """
        Recompute the best per-value match using pure lexical matching only.
        """
        if self._is_quality_feature(fg.feature_key):
            return self._quality_boost_detail(fg, source, feat_weight)
    ...
    def score_breakdown_for_hit(
        ...
        *,
        ...
        return {
            ...
            "notes": {
                "estimation": (
                    "Computed from _source values using the same unweighted feature logic "
                    "as the query builder. feature_contribution_global mirrors the actual "
                    "global dis_max-by-value feature score used for normalization. Rank "
                    "signals are diagnostic tie-breakers only and do not change _score. "
                    "ES _score may still differ if docvalues/_source differ or analyzers differ."
                ),
            },
        }
    # ----------------------------
    ...
    def build_query(self, groups: List[FeatureGroup], *, include_explain: bool = False) -> Dict[str, Any]:
        ...
        sort_clauses: List[Dict[str, Any]] = [{"_score": {"order": "desc"}}]
        if self.enable_rank_functions:
            for _, field_path, _, _, _ in self._rank_signal_specs():
                sort_clauses.append({
                    field_path: {
                        "order": "desc",
        ...
        # Do not sort on _id: Elasticsearch disallows fielddata on _id for sorting.
        # Rank signals above are used only as tie-breakers after BM25 _score.
        # If a final deterministic tie-breaker is needed, prefer a dedicated keyword field
        # such as modelID/model_id in the index mapping, not _id.
    ...
    def compute_max_score(self, groups: List[FeatureGroup]) -> float:
        ...
        for fg in groups:
            ...
            if self._is_quality_feature(fg.feature_key):
                ...
                continue
            candidate_max = float(self.MATCH_TYPE_BOOSTS["bm25"])
            for val in fg.include:
                key = self._value_key(val)
    ...
    def precompute_feature_group_cache(self, features: Any) -> List[FeatureGroup]:
        """
        Pure BM25 mode does not need synonym/gram precomputation.
        """
