...
def get_model(model_name):
    if model_name not in _GLOBAL_MODELS:
        ...
        _GLOBAL_MODELS[model_name] = SentenceTransformer(model_name)
...
class EmbeddingSynonymProvider:
    def __init__(
        ...
        normalized_features = sorted(features)
        self.candidates = self._get_candidates(normalized_features)
        self.min_similarity = min_similarity
        self.model = get_model(model_name)

        key = (model_name, tuple(normalized_features))

        if key not in _GLOBAL_EMBEDDINGS:
            _GLOBAL_EMBEDDINGS[key] = self.model.encode(
                self.candidates,
                normalize_embeddings=True,
            )

        self.candidate_embeddings = _GLOBAL_EMBEDDINGS[key]
