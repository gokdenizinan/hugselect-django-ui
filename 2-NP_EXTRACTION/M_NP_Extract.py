...
from Z_Org_Filter import process_np_dict, filter_merged_np_dict, fuzzy_merge_after_canonical_conservative
from Z_Filter import cap_model_features_np, choose_topk_log_scaled, cap_model_features_np, filter_np_entries, choose_topk_log_scaled
import pandas as pd
import spacy
import emoji
...
def remove_noisy_lines_conservative(text: str, keep_short_lines: bool = True) -> str:
    out_lines = []
    ...
    for raw in (text or "").splitlines():
        ...
        # obvious table rule lines
        if TABLE_RULE_LINE_RE.match(s):
            continue

        # numeric dumps (very strict)
        if is_mostly_numeric(s):
            continue

        # large json/yaml blobs only
        if looks_like_big_struct_blob(s):
            continue

        # optional short-line dropping
...
def preprocess_text(
    ...
    # Preserve example separators
    cleaned = re.sub(r"\s*////\s*", "\n\n", cleaned)

    # --- URLs ---
    if remove_urls:
        cleaned = replace_urls_preserving_hf(cleaned)
    ...
    if remove_inline_code:
        # keep content, remove backticks
        cleaned = re.sub(r"`([^`]+)`", r"\1", cleaned)

    # --- Line filtering ---
    if drop_noisy_lines:
        cleaned = remove_noisy_lines_conservative(
            cleaned,
...
def build_global_np_dictionary_raw(
    extractor,
    directory: str,
    description_key: str = "description",
    file_suffix: str = ".json",
    min_keep: int = 1,          # keep everything by default
    sample_fraction: float = 1.0 # 1.0 = 100%, 0.01 = 1%
):
    ...
    for filename in tqdm(files, desc="Processing files", unit="file"):
        ...
        for entry in np_entries:
            ...
            if lbl and lbl != "O":
                agg[np_text]["entity_labels"][lbl] += 1

    # apply min_keep + convert sets to lists (JSON-safe)
    out = {}
    i = 1
    for np_text, v in agg.items():
        if v["count"] < int(min_keep):
            continue
        out[f"NP_{i}"] = {
            "noun_phrase": np_text,
            "count": v["count"],
            ...
            "entity_labels": dict(v["entity_labels"]),
        }
        i += 1
...
# ['Stable Diffusion model', 'Fantasy Sword concept', 'Dreambooth', 'instance_prompt', 'photo', 'Mirage', 'text-to-3D', 'Discord Server', 'image']
    ...
    # np_dict = build_global_np_dictionary_raw(
    #     extractor,
    #     directory="HF-Models-T6",
    # )

    raw_np_dict = build_global_np_dictionary_raw(
        extractor,
        directory="HF-Models-T6",
        min_keep=1,
        sample_fraction=1,   # or 0.01 for 1%
    )

    output_path = "2-NP_EXTRACTION/NP_X1.json"
    with open(output_path, "w", encoding="utf-8") as f:
        json.dump(raw_np_dict, f, indent=2, ensure_ascii=False)

    merged_ordered, phrase_list = process_np_dict(raw_np_dict)

    output_path = "2-NP_EXTRACTION/NP_X2.json"
    with open(output_path, "w", encoding="utf-8") as f:
        json.dump(merged_ordered, f, indent=2, ensure_ascii=False)

    merged_fuzzy = fuzzy_merge_after_canonical_conservative(
    merged_ordered,
    base_threshold=90,
    anchor_min_count=5,
)

    output_path = "2-NP_EXTRACTION/NP_X3.json"
    with open(output_path, "w", encoding="utf-8") as f:
        json.dump(merged_fuzzy, f, indent=2, ensure_ascii=False)

    filtered = filter_merged_np_dict(
        merged_fuzzy,
        min_count=10,
        min_models=5,
        min_authors=2,
    ...
    output_path = "2-NP_EXTRACTION/NP_X4.json"
    with open(output_path, "w", encoding="utf-8") as f:
    ...
    #OPTIONAL NUMBER OF NPS PER MODEL CAPPING
...
# )
    ...
    print(len(raw_np_dict))
    print(len(merged_ordered))
    print(len(merged_fuzzy))
    print(len(filtered))
    print(len(filtered_np_info))
    output_path = "2-NP_EXTRACTION/NP_X5.json"

    with open(output_path, "w", encoding="utf-8") as f:
        json.dump(filtered_np_info, f, indent=2, ensure_ascii=False)
