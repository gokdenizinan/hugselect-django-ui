...
def extract_noun_phrases(text):
    ...
    for chunk in doc.noun_chunks:
        ...
        # if is_multiword(phrase, multi_word_only=True):
        #     filter_counts["multiword"] += 1
        #     good_np += 1
        # if is_bad_phrase(context):
        #     filter_counts["bad_phrase"] += 1
        #     good_np += 1  # Reset if it's a bad phrase
...
plt.bar(failed_labels, failed_counts, color='salmon', label='Passed filters')
...
plt.title('Noun Phrase Filter Criteria')
plt.xticks(rotation=30, ha='right')
plt.legend(loc='upper right', bbox_to_anchor=(1.02, 1))
plt.tight_layout()
plt.savefig('np_filter.png', dpi=300)
plt.show()


# Save results
with open("2-NP_EXTRACTION/NP_full.json", "w", encoding="utf-8") as out_file:
