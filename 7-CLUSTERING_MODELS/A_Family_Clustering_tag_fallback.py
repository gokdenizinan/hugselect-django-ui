#!/usr/bin/env python3MODEL_TYPE_COUNTS
# -*- coding: utf-8 -*-
...
your_dict = {
  "LLaMA": [
    "llama", "llama4", "mllama", "tinyllama", "longllama",
    "llava", "llava_phi", "llava_jamba", "llava_next", "llava_next_video",
    "llava_onevision", "llava_llama", "llava_qwen2", "llava_qwen",
    "llava_qwen1_5", "llava_mistral", "llava_phi3", "llava_mpt",
    "llava_cohere", "llava_gemma", "llava_stablelm_epoch",
    "llava_mini_llama", "llava_mixtral", "llava_crystal",
    "llava_vistral", "llava-chat-vector", "video_llava",
    "tinyllava", "tiny_llava", "tiny_llava_phi", "tiny_llava_stablelm",
    "vipllava", "bunny-llama", "bunny-phi", "bunny-phi3",
    "moe_llava_phi", "moe_llava_qwen", "moe_llava_qwen1_5",
    "moe_llava_mistral", "moe_llava_stablelm",
    "vila_u_llama", "mixsense_llama", "multimodal_llama",
    "spatiallm_llama", "ferret_llama", "lamed_llama"
  ],

  "Qwen": [
    "qwen", "qwen2", "qwen3", "qwen2_vl", "qwen2_5_vl", "qwen2_audio",
    "qwen2_moe", "qwen3_moe", "qwen2moe", "qwen2_vl_vpt",
    "agent_qwen2_vl", "videollama3_qwen2", "videollama2_qwen2",
    "videollama3_qwen2", "rexseek_qwen", "emova_qwen2",
    "ola_qwen", "oryx_qwen", "cambrian_qwen", "bunny-qwen",
    "bunny-qwen2", "mini_gemini_qwen2", "euclid_qwen2",
    "HumanOmni_qwen2", "vita-Qwen2", "clip_qwen2vl"
  ],

  "BERT": [
    "bert", "distilbert", "albert", "deberta", "deberta-v2",
    "electra", "mobilebert", "convbert", "megatron-bert",
    "squeezebert", "funnel", "rembert", "roc_bert",
    "bert_crf", "bert_vits2", "bert_vae", "BERT_CRF",
    "retrieva-bert", "nomic_bert", "m2_bert", "rnabert",
    "pubmedbert-bio-ext-summ", "cxr-bert", "layoutlm",
    "layoutlmv2", "layoutlmv3"
  ],

  "RoBERTa": [
    "roberta", "xlm-roberta", "xlm-roberta-xl",
    "roberta-prelayernorm", "camembert", "flaubert", "flaubert2"
  ],

  "GPT": [
    "gpt2", "gptj", "gpt_neo", "gpt_neox", "gpt_bigcode",
    "gpt_neox_japanese", "gpt_pangu", "gpt2a", "gpt2l",
    "gpt_jx", "gpt_refact", "openai-gpt", "re_gpt",
    "custom_gpt", "gpt_neox_reward_model"
  ],

  "T5": [
    "t5", "mt5", "longt5", "umt5", "mrt5",
    "codet5p", "codet5p_bimodal", "CodeT5",
    "bigbird_pegasus", "pegasus_x", "flash_t5", "fairseq_t5"
  ],

  "Mistral": [
    "mistral", "mixtral", "mistral3", "mistral-lmm",
    "vmistral", "evomistral", "turbosparsemixtral",
    "mixtral", "videollama2_mistral"
  ],

  "Gemma": [
    "gemma", "gemma2", "gemma3", "gemma3_text",
    "gemma3mm", "recurrent_gemma", "cost_wise_gemma",
    "shieldgemma2", "cerule-gemma"
  ],

  "Phi": [
    "phi", "phi2", "phi3", "phi3_v", "phi3small",
    "phi-msft", "phi4mm", "Phi-3", "phi3_v_moe"
  ],

  "ViT": [
    "vit", "vitpose", "vit_b", "vit_msn", "vit_mae",
    "vit-hybrid", "vitmatte"
  ],

  "CLIP": [
    "clip", "clipseg", "jina_clip", "altclip",
    "chinese_clip", "custom-clip-model", "clip_text_model",
    "clip_vision_model", "clip-encoder-decoder"
  ],
  ...
  "Whisper": [
    "whisper", "distilwhisper", "lite-whisper"
  ],
  ...
  "InternLM": [
    "internlm", "internlm2", "internlm3", "internlmxcomposer2",
    "InternLMXComposer"
  ],

  "Baichuan": [
    "baichuan", "baichuan_m1"
  ],
  ...
  "EfficientNet": [
    "efficientnet", "efficientnetv25", "EfficientNetB0"
  ],

  "Swin": [
    "swin", "swinv2", "swin_transformer"
  ],
  ...
  "DETR": [
    "detr", "deformable_detr", "dab-detr", "conditional_detr"
  ],
  ...
  "OLMo": [
    "olmo", "olmo2", "olmoe", "hf_olmo"
  ],

  "RWKV": [
    "rwkv", "rwkv5", "rwkv6", "rwkv7", "RWKV-6", "rwkv_hybrid"
  ],
  ...
  "Granite": [
    "granite", "granitemoe", "granite_speech"
  ],
  ...
  "Longformer": [
    "longformer", "longformer-bio-ext-summ"
  ]
}

def build_model_type_direct_map(family_to_model_types: dict) -> dict:
    """
    Convert {family: [model_types]} → MODEL_TYPE_DIRECT_MAP

    Output format:
        {normalized_model_type: (family_root, "")}
    """
    ...
    for family, model_types in family_to_model_types.items():
        for mt in model_types:
            key = normalize(mt)

            # avoid overwriting if duplicate appears across families
            if key not in mapping:
                mapping[key] = (family, "")

    return mapping
MODEL_TYPE_DIRECT_MAP = build_model_type_direct_map(your_dict)
...
TAG_WRAPPER_SUFFIXES = {
    "pipeline", "pipelines", "diffuser", "diffusers", "model", "models",
    "checkpoint", "checkpoints", "adapter", "adapters", "annotator", "annotators",
}


MANUAL_TAG_CHILD_HINTS = {
    "stablediffusionxlpipeline": ("Stable Diffusion", "SDXL"),
    "stable-diffusion-xl-pipeline": ("Stable Diffusion", "SDXL"),
    "stable-diffusion-xl-diffusers": ("Stable Diffusion", "SDXL"),
    "stable-diffusion-xl-lightning": ("Stable Diffusion", "SDXL"),
    "stable-diffusion-xl-turbo": ("Stable Diffusion", "SDXL"),
    "stable-diffusion-3-5-large": ("Stable Diffusion", "SD 3.5"),
    "stable-diffusion-3-5-large-turbo": ("Stable Diffusion", "SD 3.5"),
    "stable-diffusion-2-1": ("Stable Diffusion", "SD 2.1"),
    "stable-diffusion-1-5": ("Stable Diffusion", "SD 1.5"),
    "stable-video-diffusion": ("Stable Diffusion", "Stable Video Diffusion"),
}


def normalize_tag_value(tag: str) -> str:
    s = split_camel_case(tag)
    s = s.replace("_", "-")
    s = s.replace("/", "-")
    s = re.sub(r"[^A-Za-z0-9\-\+\.]+", "-", s)
    s = re.sub(r"-+", "-", s).strip("-")
...
def expand_tag_forms(tag: str) -> List[str]:
    raw = safe_str(tag).strip().strip("\"'")
    if not raw:
        return []

    forms = set()
    raw_norm = normalize_tag_value(raw)
    if raw_norm:
        forms.add(raw_norm)
        forms.add(raw_norm.replace("-", " "))

    spaced = split_camel_case(raw)
    spaced_norm = normalize_model_name(spaced)
    if spaced_norm:
        forms.add(spaced_norm)
        forms.add(spaced_norm.replace("-", " "))
    ...
    for form in list(forms):
        nm = normalize_model_name(form)
        if not nm:
            continue
        toks = [t for t in nm.split("-") if t]
        while toks and toks[-1] in TAG_WRAPPER_SUFFIXES:
            toks = toks[:-1]
            stripped = "-".join(toks)
            if stripped:
                forms.add(stripped)
...
def get_row_tag_forms(row: pd.Series) -> List[str]:
    seen = set()
    out = []
    for tag in parse_listish(row.get("tags", "")):
        for form in expand_tag_forms(tag):
            if form not in seen:
                seen.add(form)
                out.append(form)
    return out
...
def extract_candidate_from_tags(row: pd.Series, alias_to_family: Dict[str, str]) -> str:
    for form in get_row_tag_forms(row):
        nm = normalize_model_name(form)
        if not nm:
            continue

        if nm in MANUAL_TAG_CHILD_HINTS:
            return nm
        ...
        cand = extract_candidate_from_name(nm)
        if cand and is_valid_family_candidate(cand, alias_to_family):
            return cand

        if nm in alias_to_family:
            return nm
...
def extract_candidate_root(row: pd.Series, alias_to_family: Dict[str, str]) -> str:
    ...
    tag_cand = extract_candidate_from_tags(row, alias_to_family)
    if tag_cand and is_valid_family_candidate(tag_cand, alias_to_family):
        return tag_cand
...
def build_row_text_for_match(row: pd.Series) -> str:
    tag_text = " ".join(parse_listish(row.get("tags", "")))
    tag_forms = " | ".join(get_row_tag_forms(row))
    pieces = [
...
def known_family_match(
    ...
    tag_forms = get_row_tag_forms(row)
    ...
    for tag_form in tag_forms:
        nm = normalize_model_name(tag_form)
        if not nm:
            continue

        if nm in MANUAL_TAG_CHILD_HINTS:
            parent, child = MANUAL_TAG_CHILD_HINTS[nm]
            return FamilyMatch(
                family_root=parent,
                family_child=child,
                method="tag_child_pattern",
                confidence=0.92,
                evidence={"matched_on_tag": tag_form}
            )
        ...
        if nm in alias_to_family:
            fam = alias_to_family[nm]
            parent = catalog.get(fam, {}).get("parent", "")
            return FamilyMatch(
                family_root=parent if parent else fam,
                family_child=fam if parent else "",
                method="tag_alias_exact",
                confidence=0.88,
                evidence={"matched_on_tag": tag_form}
            )
...
def main():
    ...
    args = parser.parse_args([
        ...
        "--out_assignments", "7-CLUSTERING_MODELS/clusters_improved_2/family_assignments.csv",
        "--out_relations", "7-CLUSTERING_MODELS/clusters_improved_2/family_relations.csv",
        "--out_candidates", "7-CLUSTERING_MODELS/clusters_improved_2/discovered_family_candidates.csv"
    ])
