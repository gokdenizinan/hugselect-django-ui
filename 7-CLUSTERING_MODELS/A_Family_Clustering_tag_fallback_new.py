...
your_dict = {
  ...
  "AudioLDM": ["diffusers:AudioLDMPipeline", "diffusers:AudioLDM2Pipeline"],
  ...
  "chinese_clip",
  "japanese_clip",
  "japanese_stable_clip",
  "hybrid_clip",
  "clipseg",
  "clip_encoder_decoder",
  "clip_vision_bert",
  "clip_vision_mbart",
  "clip_qwen2vl",
  "clip_text_camembert",
...
def extract_candidate_root(row: pd.Series, alias_to_family: Dict[str, str]) -> str:
    ...
    """
    for field in ["base_model", "model_type", "model_name", "model_id"]:
...
def known_family_match(
    ...
    domain = safe_str(row.get("assigned_domain", ""))
...
def candidate_base_model_hit_rate(df: pd.DataFrame, cand: str) -> float:
    if df.empty:
        return 0.0
    vals = df["base_model"].fillna("").astype(str).map(normalize_model_name)
    return float(vals.str.contains(re.escape(cand), regex=True).mean())
...
def promote_discovered_candidates(
    ...
    domain_counts = (
        work.groupby(["assigned_domain", "candidate_root_norm"])
        .size()
    ...
    for _, r in merged.iterrows():
        ...
        domain = safe_str(r["assigned_domain"])
    ...
    out = out.sort_values(
        ...
        subset=["assigned_domain", "candidate_root_norm", "canonical_or_discovered_root"]
    )
...
def build_family_relations(
    ...
    for fam, cfg in catalog.items():
        ...
        if parent:
            rows.append({
                ...
                "assigned_domain": "|".join(cfg.get("domain_hints", [])),
            })
    ...
    if discovered_df is not None and not discovered_df.empty:
        for _, r in discovered_df.iterrows():
            ...
            domain = safe_str(r.get("assigned_domain", ""))
    ...
    if not rows:
        return pd.DataFrame(columns=["parent_family", "child_family", "relation_source", "assigned_domain"])
...
def inherit_from_base_model(row, model_family_lookup: Dict[str, Dict[str, str]]) -> Optional[Dict[str, str]]:
    """
    If this row is unresolved but has base_model pointing to a model with a known family,
    inherit that family.
    """
    base_model = parse_listish(row.get("base_model", ""))
...
def assign_with_discovery(
    ...
    for _, row in df.iterrows():
        ...
        # NEW: inherit from base_model if possible
        inherited = inherit_from_base_model(row, model_family_lookup)
        if inherited:
...
def main():
    ...
    print("uploaded csv")
    if "assigned_domain" not in df.columns:
        raise ValueError("Missing required column: assigned_domain")
