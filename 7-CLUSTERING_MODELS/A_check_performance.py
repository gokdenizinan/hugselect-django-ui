...
CSV_PATH = "7-CLUSTERING_MODELS/clusters_improved_2/family_assignments_organized.csv"
OUT_DIR = Path("7-CLUSTERING_MODELS/cluster_counts")
OUT_DIR.mkdir(parents=True, exist_ok=True)

df = pd.read_csv(CSV_PATH)


def is_unclear(value):
    return isinstance(value, str) and value.strip().lower() == "other / unclear"


def count_and_save(column_name, filename):
    series = df[column_name].dropna()

    # Remove "Other / Unclear"
    series = series[~series.apply(is_unclear)]
...
# ---- Run ----
count_and_save("assigned_domain", "assigned_domain_counts.json")
count_and_save("family_root", "family_root_counts.json")
count_and_save("family_child", "family_child_counts.json")
...
# df_full = pd.read_csv("7-CLUSTERING_MODELS/clusters_improved_2/family_assignments_organized.csv")
...
# # # ---- Save to JSON ----
# with open("7-CLUSTERING_MODELS/cluster_counts/family_root_counts.json", "w", encoding="utf-8") as f:
...
# with open("7-CLUSTERING_MODELS/cluster_counts/family_child_counts.json", "w", encoding="utf-8") as f:
...
# with open("7-CLUSTERING_MODELS/cluster_counts/assigned_modality_counts.json", "w", encoding="utf-8") as f:
...
# with open("7-CLUSTERING_MODELS/cluster_counts/pipeline_tag_counts.json", "w", encoding="utf-8") as f:
#     json.dump(pipeline_tag_counter, f, indent=2)
# # ============================================

# task_tag = df_full["task"].dropna()
# print("task ", len(pipeline_tag))

# task_counter = Counter(task_tag)
# with open("7-CLUSTERING_MODELS/cluster_counts/task_counts.json", "w", encoding="utf-8") as f:
#     json.dump(task_counter, f, indent=2)

# # ============================================
# model_type = df_full["model_type"].dropna()
# print("model type", len(model_type))

# baseline_model = df_full["base_model"].dropna()
# print("baseline model", len(baseline_model))
# # print(f"\nSaved family_root_counts.json with {len(family_root_counter)} unique values")

# print("")
# print(set(df_full["family_root"].dropna()))
