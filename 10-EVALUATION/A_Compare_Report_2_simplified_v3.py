...
def build_wide_details(
    ...
    *,
    ...
    prefix_map = {
        "Instruct_Features": "Inst",
        "ChatGPT_Features": "Chat",
        "Gemini_Features": "Gemi",
        "United_Features": "Unit",
    }
    ...
    for _, r in df.iterrows():
        ...
        for col in compare_cols:
            ...
            unmatched_lists[f"{pr}_feature"] = unmatched_truths
        ...
        for k in range(max_len):
            ...
            for col in compare_cols:
                pr = _prefix(col)
                feats = unmatched_lists.get(f"{pr}_feature", [])
                row[f"{pr}_feature"] = feats[k] if k < len(feats) else ""
    ...
    for col in compare_cols:
        pr = _prefix(col)
        other_cols.append(f"{pr}_feature")
...
OUTPUT_DETAILS_CSV = f"10-EVALUATION/enriched_ffs/enriched_details{FILL}.csv"
